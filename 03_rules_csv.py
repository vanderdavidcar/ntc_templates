...
# To find out all rules with "any" as a source on cisco ASA access-list
any4 = "access-list Internet-ACL.*tcp.(\S+).host.(\S+).eq.(\S+)"
any4_regex = re.findall(any4, shrun)

# To find out all rules with "host" as a source on cisco ASA access-list
host = "access-list.*tcp.host.(\S+).host.(\S+).eq.(\S+)"
host_regex = re.findall(host,shrun)

# To find out all rules with "object-group" as a source on cisco ASA access-list
...
for grp in regex_grp:
    ...
    with open('all_grpaddress.csv', 'a') as out_file:
        writer = csv.writer(out_file)
        #writer.writerow(('source', 'destination', 'port'))
        writer.writerow(grp)
